...
writeTimedBuffer buf value = atomically $ writeTimedBufferSTM buf value


writeTimedBufferSTM :: TimedBuffer a -> a -> STM ()
writeTimedBufferSTM buf value = do
    i <- readTVar (idx buf)
    if i >= (maxN buf)
        then retrySTM
        else do
