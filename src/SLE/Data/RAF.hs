...
processInitState cfg var (SlePduBind pdu) = do 
  ...
  let res = do
        ...
        if (pdu ^. sleBindInitiatorID /= auth) 
          then Left ("Access Denied, initiator now allowed: " <> display (pdu ^. sleBindInitiatorID), AccessDenied)
          else Right () 
        -- Check, if we are a RAF Bind Request
        if (pdu ^. sleBindServiceType /= RtnAllFrames)
          then Left ("Requested Service is not RAF: " <> display (pdu ^. sleBindServiceType), ServiceTypeNotSupported)
          else Right () 
        -- check the requested SLE Version 
        if (pdu ^. sleVersionNumber /= VersionNumber 3)
          then Left ("Version not supported: " <> display (pdu ^. sleVersionNumber), VersionNotSupported)
          else Right ()  
        if sii /= (cfg ^. cfgRAFSII)
          then Left ("No such service instance supported: " <> display sii, NoSuchServiceInstance)
          else Right ()
  case res of 
    Right _ -> do 
